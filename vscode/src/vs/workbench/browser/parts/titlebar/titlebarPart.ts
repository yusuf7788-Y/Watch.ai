...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		// Watch.ai Logo & Sidebar Toggle
		const watchLogo = append(this.leftContent, $('a.window-watch-logo', { title: 'Watch AI Sidebar Toggle' }));
		watchLogo.style.backgroundImage = `url('file:///C:/Watch.ai/logo.png')`;
		watchLogo.onclick = async () => {
			const isVisible = this.layoutService.isVisible(Parts.SIDEBAR_PART);
			const currentPosition = this.configurationService.getValue('workbench.sideBar.location');
