		...
		await this.throttler.queue(() => new Promise<void>((c) => {
			try {
				if (typeof createWatcher === 'function') {
					...
					this.logService.warn(`NativePolicyService#_updatePolicyDefinitions - createWatcher is not a function, skipping policy enforcement`);
					c();
				}
			} catch (err) {
				this.logService.error(`NativePolicyService#_updatePolicyDefinitions - Error creating watcher:`, err);
				c(); // Continue anyway to avoid crashing the IDE
			}
		}));
	}
